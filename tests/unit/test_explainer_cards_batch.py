...
class _CountingLLM:
    """Return a usable card body and track concurrent call count."""
...
def test_write_explainer_cards_batch_fails_without_llm() -> None:
    """Batch tool should fail early when llm_provider is None."""
    state = SectorBreakerState.initialize(project_id="project-kernel", domain="API中转站", user_goal="建库")
    context = KernelRuntimeContext(
        project=_project(),
        repository=_Repo(),  # type: ignore[arg-type]
    ...
    observation = asyncio.run(write_explainer_cards_batch(
        ToolCall(
            tool_name="write_explainer_cards_batch",
            args={"cards": [{"title": "A", "focus": "a", "card_kind": "concept", "writing_goal": "test"}]},
            reason="test",
        ),
        context,
    ))
    ...
    assert observation.error == "llm provider not configured"
