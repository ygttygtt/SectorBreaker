...
- V2 Agent Kernel recovery/UX 已落地：可选解释卡/索引/调研复盘写作失败不会再拖垮已完成主文档；主写作失败会记录 `failed_writes`，已有 artifact 时可返回 partial-success metadata。Agent 决策新增 `user_notice`，前端优先展示人话进展，不再把 `L1`、`ready_to_write=True` 这类内部参数直接给用户看。
- 运行日志与复盘已产品化：API 新增 `GET /api/runs/{run_id}/trace`，结果页可下载 JSON 思考日志；Kernel 新增 `generate_run_narrative` 工具，用第一人称生成“我是怎么调研的”复盘 artifact。
- 设置页确认感已增强：显示当前生效模型，表单改动会提示未保存，关闭前确认；选择预设只是加载到表单，应用/保存后才生效；带本地保存 Key 的预设可以留空 Key 直接应用，但不会回显明文；新输入的 Key 会保留到用户保存预设，避免刚输入就丢。
- Agent Kernel 已新增 `write_explainer_cards_batch`，用于主文档之后并行生成互不依赖的可选 Obsidian 解释卡。它复用单卡写作逻辑，返回成功 artifact ids 和失败卡片诊断，不能替代主层级文档。
...
- 当前 V2 recovery/UX 验证：`python -m pytest tests/unit/test_explainer_cards_batch.py -q` => 1 passed；`python -m pytest tests/unit/test_kernel_partial_success.py tests/unit/test_agent_kernel_models.py tests/unit/test_run_narrative.py tests/api/test_run_trace_export.py -q` => 8 passed，1 个已知 warning；Agent Kernel API failure/partial-artifact focused suite => 2 passed，1 warning；`cd frontend && npm test -- --run App.test.tsx` => 20 passed；`cd frontend && npm run build` passed，仅 Vite chunk-size warning；`python tools/check_version_isolation.py` passed。
