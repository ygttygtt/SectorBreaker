...
- V2 Agent Kernel 富知识库输出升级已落地：Master Agent 可调用 `write_explainer_card` 为概念、工具、玩家、风险、流程、问题等生成独立 Obsidian 卡片，也可调用 `write_vault_index` 生成知识库导航页。V2 导出 README 会把主文档、解释性卡片、证据账本、`.sectorbreaker/` 状态包和继续补库方式作为首页重点。后续验收不应只看是否有 5 篇主文档，还要看 Agent 是否根据 State 发现并补齐了有展示价值的概念卡/下钻卡。
- V2 搜索与写作强度反馈已修正：`search_web` 支持 `queries` variants，一次工具调用可以对同一缺口执行 2-4 条真人式短 query；search prompt 不再鼓励长关键词堆叠。writer/search 数字在 policy prompt 中是 soft strength guidance，quick 默认 writer strength 已提高，避免 `Writer Calls=5` 诱导模型早停。根目录 `.obsidian/` 已同步包含插件、主题和 snippets 的模板。
