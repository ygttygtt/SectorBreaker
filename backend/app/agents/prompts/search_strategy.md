...
- 地域、语言、时间或市场范围约束；
- 可能的专业术语、同义词或英文术语；
- 想要的来源类型，如 report、policy、case、official docs、pricing、benchmark、forum discussion。

示例：

```json
{
  "query": "API中转站 One API New API 协议转换 原理 模型网关",
  "layer_hint": "L3_how",
...
```json
{
  "query": "API中转站 定价 代理 上游供应商 成本 模型API",
...
```json
{
  "query": "API中转站 风险 合规 平台封禁 数据安全 账号滥用",
...
```json
{
  ...
  "search_intents": [
    {
      "query": "API中转站 One API New API 协议转换 模型网关 原理",
      "layer_hint": "L3_how",
