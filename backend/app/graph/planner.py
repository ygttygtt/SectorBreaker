...
def build_v1_master_workflow_definition() -> WorkflowDefinition:
    ...
    nodes = [
        WorkflowNode(
            ...
            node_type="gate",
            group="scope",
            reason="建立目标、L1-L5 认知 schema、运行记忆、工具预算和安全边界。",
        ),
        WorkflowNode(
            ...
            group="source",
            reason="把上传报告、用户材料、引用和低可信线索写入 Agent State，作为后续决策输入。",
        ),
        WorkflowNode(
            id="agent_decide",
            label="LLM 大脑决策",
            ...
            group="plan",
            reason="LLM 读取 State 与 Tools，决定下一步搜索、读材料、写作、审查、问用户或结束。",
        ),
        WorkflowNode(
            id="tool_execution",
            label="工具执行",
            ...
            group="source",
            reason="执行 search_web、read_uploaded_report、retrieve_project_memory、write_layer_document 等真实工具。",
        ),
        WorkflowNode(
            id="state_update",
            label="State 更新",
            node_type="store",
            group="evidence",
            reason="把 Observation 内化为 source memory、claims、entities、open questions、artifact refs 和 decision log。",
        ),
        WorkflowNode(
            id="artifact_writing",
            label="知识库写作",
            ...
            group="synthesis",
            reason="当 LLM 判断材料足够时，通过工具写入 V2 L1-L5 / 动态 schema Obsidian 文档。",
        ),
        WorkflowNode(
            id="artifact_review",
            label="详实度审查",
            ...
            group="qa",
            reason="检查文档是否详实、证据关联、Obsidian 友好；不足时让 Agent 决定补搜或修订。",
        ),
        WorkflowNode(
            id="human_feedback",
            label="人在回路",
            node_type="human",
            group="qa",
            reason="当边界、材料或风险不清时，Agent 可以暂停并请求用户反馈。",
        ),
        WorkflowNode(
            id="export",
            ...
            group="export",
            reason="持久化 Agent 写出的 Markdown 产物，并支持后续项目问答。",
        ),
    ]
    edges = [
        WorkflowEdge(id="v2-e-init-materials", source="initialize_state", target="external_materials", label="读取材料"),
        WorkflowEdge(id="v2-e-materials-decide", source="external_materials", target="agent_decide", label="State 输入"),
        WorkflowEdge(id="v2-e-decide-tool", source="agent_decide", target="tool_execution", label="选择工具"),
        WorkflowEdge(id="v2-e-tool-state", source="tool_execution", target="state_update", label="Observation"),
        WorkflowEdge(id="v2-e-state-loop", source="state_update", target="agent_decide", label="继续判断"),
        WorkflowEdge(id="v2-e-tool-write", source="tool_execution", target="artifact_writing", label="写作工具"),
        WorkflowEdge(id="v2-e-write-review", source="artifact_writing", target="artifact_review", label="审查"),
        WorkflowEdge(id="v2-e-review-loop", source="artifact_review", target="agent_decide", label="补搜/修订"),
        WorkflowEdge(id="v2-e-decide-human", source="agent_decide", target="human_feedback", label="需要反馈"),
        WorkflowEdge(id="v2-e-human-loop", source="human_feedback", target="agent_decide", label="反馈入 State"),
        WorkflowEdge(id="v2-e-decide-export", source="agent_decide", target="export", label="完成"),
    ]
