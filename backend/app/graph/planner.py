...
def build_v1_master_workflow_definition() -> WorkflowDefinition:
    ...
    nodes = [
        ...
        WorkflowNode(
            id="specialist_react_loop",
            label="L0-L5 分层 ReAct",
            node_type="agent",
            agent_id="specialist_react_loop",
            group="analysis",
            reason="各层 specialist 根据 ContextPack 调用工具、观察结果、更新状态并判断局部覆盖。",
        ),
        WorkflowNode(
            id="evidence_ledger",
            label="证据账本",
    ...
    edges = [
        ...
        WorkflowEdge(id="v1-e-search-specialist", source="source_collection", target="specialist_react_loop", label="分层执行"),
        WorkflowEdge(id="v1-e-specialist-ledger", source="specialist_react_loop", target="evidence_ledger", label="结构化记忆"),
