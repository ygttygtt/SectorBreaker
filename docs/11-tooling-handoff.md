...
- V2 Agent Kernel rich-vault output is now an explicit product requirement. The Agent can call `write_explainer_card` for concepts, tools, players, risks, processes, notes, and drill-down questions, and `write_vault_index` to create a connected Obsidian navigation page. The V2 exporter now writes a V2-specific README that makes auxiliary cards visible. Future agents should not judge demo readiness by "five main docs exist" alone; inspect whether the run produced useful linked cards when State revealed beginner blockers or sub-concepts.
- V2 search and writing-strength follow-up: `search_web` supports `queries` variants and the search prompt asks for short human-style query phrases such as `API 中转站 原理` / `API 中转站 头部平台`, not long keyword bags. Writer/search counts in the policy prompt are soft strength hints, not hard stop criteria; quick runs now have more writer strength for richer exports. The root `.obsidian/` template includes the user's plugin/theme/snippet configuration for future vault exports.
- V2 demo-readiness note: do not reintroduce hard workflow-like search/write caps. Current Kernel uses decision heartbeats, Artifact Memory in policy context, faster one-shot artifact writing, and open-web partial-evidence tolerance. DeepSeek real short checks show visible Agent reasoning and real artifact writing, but full completion may still run long because the Agent keeps researching. For recording, either use a faster configured LLM such as Mimo for acceptance or frame the demo around the live Agent process and exported partial artifacts.
- V2 running-page UX recovery note: the workbench now stores active `project/run` in URL/localStorage and restores the run from snapshot after refresh. Main Agent brief filters zero-delta State Updates, summarizes meaningful State Updates, collapses secondary details, and auto-scrolls new cards. The right raw log is viewport-constrained, and the workflow minimap is hidden by default because it was visually broken in the current React Flow layout.
