...
- V2 foundation implementation has started without replacing the stable V1.6 run path. Implemented modules: `backend/app/agent_state/models.py` for `SectorBreakerState`, dynamic L0-L5 `KnowledgeSchema`, `TaskMemory`, `ContextPack`, and decisions; `backend/app/agent_state/context_pack.py` for context retention/filtering; `backend/app/agent_state/report_internalizer.py` for DeepSearch report claims/entities/open questions/source memory; `backend/app/agents/react_loop.py` for bounded Thought/Action/Observation loops; `backend/app/agents/specialists.py` for L1-L5 specialist contracts and recursive follow-up task discovery; `backend/app/agents/iceberg_agent.py` for safe risk/iceberg signal extraction and L4/L5 state objects; `backend/app/graph/v2_react_graph.py` for a side-by-side LangGraph skeleton with conditional routing. This is real code with tests, but the production API still uses the V1.6 path until the V2 graph is wired into runs.
...
V2 foundation status: state/memory models, ContextPackBuilder, report
internalizer, generic bounded ReAct runner, L1-L5 specialist contracts,
recursive follow-up task discovery, safe iceberg/risk extraction, and a
side-by-side V2 LangGraph skeleton are implemented and tested. Next work is to
wire these modules into the real run API, persist V2 state, execute specialist
loops with real LLM/tool policies, and add human-feedback reopening.
