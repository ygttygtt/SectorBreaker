...
- Legacy V1/V2 workflow code is now isolated under `backend/app/legacy/` as `legacy_v1_pipeline.py` and `legacy_fixed_v2_pipeline.py`. Production code outside `backend/app/legacy/` must not import this package; it exists only for historical comparison and legacy unit tests.
