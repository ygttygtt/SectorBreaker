...
- Current V1.5 focused verification: `python -m pytest tests/unit/test_v1_pipeline.py -q` => 14 passed; upload/export API subset => 6 passed, 1 warning; `cd frontend && npm test -- --run App.test.tsx` => 17 passed; `cd frontend && npm run build` passed with the existing Vite chunk-size warning.
