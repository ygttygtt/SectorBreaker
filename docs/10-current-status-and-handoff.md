...
- V2 foundation code still exists as a legacy tested path in `backend/app/v2_pipeline.py`, but it is no longer the production personal auto-run entry. Keep it for comparison/tests only unless deliberately removed in a later cleanup.
- V2 Agent Kernel is now the production personal `domain_knowledge` auto-run path. `backend/app/agent_kernel/pipeline.py` initializes `SectorBreakerState`, internalizes uploaded reports/materials, lets the LLM policy choose from approved tools, applies `KernelStateDelta`, and persists artifacts only when the kernel completes. The event stream exposes Thought Summary / Action / Observation / State Update / Decision messages. `write_layer_document` retries LLM Markdown generation up to three times; if writing still fails or is too thin, the run fails with `artifact_writing_failed` and no fake template artifact is saved. The older `backend/app/v2_pipeline.py` must not be reconnected as production V2.
