...
- V1.3 planning now targets `Talent Demand Intelligence Agent` as the next enterprise-oriented direction. The implementation plan is `docs/superpowers/plans/2026-07-04-v1-3-talent-demand-intelligence.md`; it is additive and must preserve the existing V1.2 domain-knowledge flow while adding talent-demand mode, JD/report ingestion, skill normalization, source coverage matrix, talent-specific Obsidian export, and source/legal guardrails.
