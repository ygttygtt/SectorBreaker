...
- V2 foundation is now wired into the personal `domain_knowledge` auto-run path through `backend/app/v2_pipeline.py`. Implemented modules: `backend/app/agent_state/models.py` for `SectorBreakerState`, dynamic L0-L5 `KnowledgeSchema`, `TaskMemory`, `ContextPack`, and decisions; `backend/app/agent_state/context_pack.py` for context retention/filtering; `backend/app/agent_state/report_internalizer.py` for DeepSearch report claims/entities/open questions/source memory; `backend/app/agents/react_loop.py` for bounded Thought/Action/Observation loops; `backend/app/agents/specialists.py` for L1-L5 specialist contracts and recursive follow-up task discovery; `backend/app/agents/iceberg_agent.py` for safe risk/iceberg signal extraction and L4/L5 state objects; `backend/app/graph/v2_react_graph.py` for a side-by-side LangGraph skeleton with conditional routing. The real API now calls V2 for personal auto-runs, while the final Markdown writer still reuses the stable V1 artifact builder.
