...
- V2 Agent Kernel rich-vault upgrade adds Agent-selectable `write_explainer_card` and `write_vault_index` tools. The Master Agent is now prompted to treat main layer documents as the cognition backbone, then decide from State whether beginner blockers, technical terms, implementation processes, players, risks, or unresolved drill-down questions deserve independent Obsidian cards. V2 exports now generate a V2-specific README that foregrounds main docs, cards, the generated navigation page, evidence ledger, and `.sectorbreaker/` state bundle. Acceptance should no longer treat "five main docs only" as the desired ceiling; a good demo vault should include useful auxiliary cards when the domain reveals them.
- V2 Agent Kernel search and writing-strength feedback follow-up: `search_web` now accepts `queries` variants so the Agent can issue 2-4 human-style searches for one missing dimension instead of packing many keywords into one long query. Runtime guidance now labels iteration/search/writer counts as soft strength hints, and quick projects use a much larger writer strength so `Writer Calls=5` no longer suppresses richer vault output. The repository-root `.obsidian/` template now includes the user's plugin/theme/snippet configuration and should be copied into new exports.
...
- V2.0 Agent Kernel is the current code reality for personal `domain_knowledge`; older V1.6 workflow wording is historical context, not the active execution model. V2.0 now persists `SectorBreakerState` checkpoints, injects universal anchor layers, exports main documents under `docs/` and auxiliary cards under `cards/`, supports `revise_layer_document`, and exposes `POST /api/projects/{project_id}/continue` for follow-up expansion.
- V2.0 checkpoint continuation fix: `/continue` now loads the latest resumable checkpoint by `project_id` instead of assuming `run_id=project_id`. This matters because follow-up runs save checkpoints under their own run ids. Completed checkpoints use `run_end_completed` and artifact checkpoints use `artifact_write`; diagnostic `run_end` checkpoints from non-completed runs are not default continuation sources.
- Local LLM provider switching is now preset-based. The backend stores private LLM presets in local runtime config, exposes list/upsert/delete/apply routes, never returns API keys in preset list responses, and `.gitignore` excludes `*.runtime-config.json`. The settings UI can select built-in DeepSeek / SenseNova V4 Flash / Mimo templates, create user presets, apply a preset with a local key, test the connection, and keep search settings in a cleaner card layout.
...
- Current V2.0 checkpoint/continue verification: `python -m pytest tests/unit/test_run_state_checkpoint.py tests/api/test_app.py::test_api_continue_uses_latest_project_checkpoint_after_previous_continue tests/unit/test_agent_kernel_runtime.py tests/unit/test_agent_kernel_tools.py tests/unit/test_markdown_exporter.py tests/unit/test_agent_kernel_schema_planner.py -q` => 27 passed, 1 known Starlette/TestClient warning; `python tools/check_version_isolation.py` passed; `cd frontend && npm test -- --run App.test.tsx` => 19 passed.
- Current local LLM preset/settings verification: `python -m pytest tests/api/test_app.py::test_api_restores_persisted_llm_runtime_config_after_restart tests/api/test_app.py::test_api_manages_local_llm_presets_without_uploading_to_repo tests/api/test_app.py::test_api_restores_persisted_search_runtime_config_after_restart -q` => 3 passed, 1 known Starlette/TestClient warning; `cd frontend && npm test -- --run App.test.tsx` => 20 passed; `cd frontend && npm run build` passed with the existing Vite chunk-size warning.
- Current V2 Agent Kernel recovery/UX follow-up: optional explainer-card/index/narrative write failures no longer discard completed main artifacts; main writer failures keep failed-write diagnostics and return partial-success metadata when artifacts exist. The policy schema now has `user_notice`, and the frontend prefers that plain-language notice over raw internal parameters for live Agent brief cards. The API exposes `GET /api/runs/{run_id}/trace`, results can download the JSON trace, and the Agent has a `generate_run_narrative` tool for a first-person research recap artifact.
- Current settings UX follow-up: the LLM settings panel now distinguishes "loaded preset" from "currently effective model", warns before closing unsaved LLM changes, allows applying a preset that has a locally stored key without exposing the key, keeps newly typed keys available for immediate preset saving, and uses a slimmer modal scrollbar. Built-in and user-created presets remain local runtime config and must not be committed.
- Current parallel-card follow-up: Agent Kernel now registers `write_explainer_cards_batch` for independent optional Obsidian cards. It reuses the existing single-card writer and returns aggregate artifact IDs/failures, so the Master Agent can create supplementary cards more efficiently without turning the main run into a fixed workflow.
- Current V2 recovery/UX verification: `python -m pytest tests/unit/test_explainer_cards_batch.py -q` => 1 passed; `python -m pytest tests/unit/test_kernel_partial_success.py tests/unit/test_agent_kernel_models.py tests/unit/test_run_narrative.py tests/api/test_run_trace_export.py -q` => 8 passed, 1 known Starlette warning; focused Agent Kernel API failure/partial-artifact suite => 2 passed, 1 warning; `cd frontend && npm test -- --run App.test.tsx` => 20 passed; `cd frontend && npm run build` passed with the existing Vite chunk-size warning; `python tools/check_version_isolation.py` passed.
